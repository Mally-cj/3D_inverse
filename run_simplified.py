...
#############################################################################################################
    ...
    with torch.no_grad():
        for i, (s_patch, imp_patch, zback_patch) in enumerate(test_loader):
            ...
            print(f"   处理patch {i*Z_pred_np.shape[0]+1}/{len(indices)}")
    
    # 3. 拼回3D体
    pred_patches = np.stack(pred_patch_list)  # [N, time, patch_size]
    pred_patches = pred_patches[:, None, :, :]  # [N, 1, time, patch_size]
    patch_size = pred_patches.shape[-1]
    pred_3d = processor.reconstruct_3d_from_patches(pred_patches, indices, shape3d, patch_size, axis=0)
    true_3d = processor.reconstruct_3d_from_patches(true_patch_list, indices, shape3d)
    back_3d = processor.reconstruct_3d_from_patches(back_patch_list, indices, shape3d)
    seismic_3d = processor.reconstruct_3d_from_patches(seismic_patch_list, indices, shape3d)
    ...
    np.save('logs/results/seismic_record.npy', np.array(seismic_patch_list))
